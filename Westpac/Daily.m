    ...
    if ([receivedData isKindOfClass:[NSArray class]]) {
        for (NSDictionary *item in (NSArray *)receivedData) {
            if ([item isKindOfClass:[NSDictionary class]]) {
                [parsedData addObject:[ForecastData modelObjectWithDictionary:item]];
            }
       }
    } else if ([receivedData isKindOfClass:[NSDictionary class]]) {
       [parsedData addObject:[ForecastData modelObjectWithDictionary:(NSDictionary *)receivedData]];
    }

    self.forecastData = [NSArray arrayWithArray:parsedData];
...
- (NSDictionary *)dictionaryRepresentation
{
    ...
    for (NSObject *subArrayObject in self.forecastData) {
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    self.forecastData = [aDecoder decodeObjectForKey:kDailyData];
    return self;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    [aCoder encodeObject:_forecastData forKey:kDailyData];
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    if (copy) {
        ...
        copy.forecastData = [self.forecastData copyWithZone:zone];
    }
